...
class Calculations:
    ...
    def compute_circuits(self):
        circuits = []
        for distance in self.distances[0:self.iterations]:
        ...
        self.circuits = sorted(circuits, key=lambda circuit: circuit.size(), reverse=True)

    def product_of_largest_circuits(self):
        total = 1
        for circuit in self.circuits[0:3]:
            total *= circuit.size()
        return total

print(Calculations('example', 10).product_of_largest_circuits())
print(Calculations('input', 1000).product_of_largest_circuits())
